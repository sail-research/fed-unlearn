...
if __name__ == "__main__":
    ...
    if not args.is_onboarding:
        ...
        end_round = (
            args.num_rounds + args.num_unlearn_rounds + args.num_post_training_rounds
        )

        res = get_results(args)

        for round in range(end_round):
            if round == num_rounds:
            ...
            print(
                "Round {}/{}: lr {} {}".format(
                    round + 1, end_round, args.lr, args.out_file
                )
            )
            ...
            save_param(args, param=global_param, case=1, round=round)
        ...
        global_param = torch.load(
            f"./results/models/case1/case1_{args.dataset}_C{args.num_clients}_BS{args.batch_size}_R{args.num_rounds}_UR{args.num_unlearn_rounds}_PR{args.num_post_training_rounds}_E{args.local_epochs}_LR{args.lr}_round{start_round-1}.pt"
        )

        res = load_results(
            f"./results/case1_{args.dataset}_C{args.num_clients}_BS{args.batch_size}_R{args.num_rounds}_UR{args.num_unlearn_rounds}_PR{args.num_post_training_rounds}_E{args.local_epochs}_LR{args.lr}.pkl"
        )
        ...
        for round in range(start_round, end_round):
            ...
            # print("debug check time:", res["time"])
